...
function ProfileContent() {
  ...
  const loadProfile = async () => {
    try {
      ...
      if (data.user) {
        ...
        setMinecraftUsername(data.user.minecraftUsername || "");
  ...
  const isPartnerOrAdmin = profileData?.role === "PARTNER" || profileData?.role === "ADMIN";
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-white/[0.08] pb-4">
        <div className="flex items-center space-x-3.5">
          ...
          <div className="relative">
            <img
              src={getMinecraftHeadRender(minecraftUsername || profileData?.email || "Steve")}
              alt="Player Head"
              className="w-12 h-12 rounded-xl border border-white/[0.1] object-contain shadow-md"
            />
          </div>
          <div>
            <div className="flex items-center space-x-2">
              <h1 className="text-xl font-bold text-white tracking-tight">
                {profileData?.name || minecraftUsername || "Mi Perfil"}
              </h1>
            ...
            <p className="text-xs text-zinc-400 font-mono mt-0.5">{profileData?.email}</p>
          </div>
        </div>
        ...
        {profileData?.role === "ADMIN" && (
          <Link
            href="/admin"
            className="px-3.5 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 rounded-lg text-xs font-bold transition-all flex items-center space-x-1.5 self-start sm:self-auto"
          >
      ...
      {/* Tabs */}
      <div className="flex items-center space-x-2 border-b border-white/[0.08] pb-1 overflow-x-auto scrollbar-none">
        <button
          onClick={() => setActiveTab("account")}
          className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-semibold text-xs transition-all ${
            activeTab === "account"
              ? "bg-white/[0.08] text-white border-b-2 border-amber-500 shadow-sm"
      ...
      {activeTab === "account" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 bg-[#0e1017] border border-white/[0.08] rounded-xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-white mb-4 flex items-center space-x-2">
              <User className="w-4 h-4 text-amber-400" />
              <span>Información Personal & Minecraft</span>
            </h3>
            ...
            <form onSubmit={handleUpdateProfile} className="space-y-4">
              ...
              <div className="bg-[#090a0f] border border-white/[0.08] rounded-xl p-4 space-y-3">
                <span className="text-xs font-bold text-zinc-200 block">
                  Perfil de Minecraft (Recepción de Compras)
                </span>
          ...
          {/* Social Accounts & Linking Side Card */}
          <div className="space-y-4">
            <div className="bg-[#0e1017] border border-white/[0.08] rounded-xl p-5 shadow-sm space-y-3.5">
              ...
              <p className="text-xs text-zinc-400 leading-relaxed">
                Vincula tu Discord y Google para iniciar sesión rápidamente y tener tu cuenta unificada.
              </p>
              ...
              <div className="bg-[#090a0f] border border-white/[0.08] rounded-lg p-3 flex items-center justify-between">
                <div className="flex items-center space-x-2.5">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-zinc-400">{profileData?.email}</div>
                  </div>
                </div>
                <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 text-[10px] font-bold rounded">
                  Activo
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="bg-[#0e1017] border border-white/[0.08] rounded-xl p-4 shadow-sm relative overflow-hidden group border-amber-500/30">
              ...
              <div className="text-[11px] text-emerald-400 font-semibold mt-1">
                Disponible para pago / canje
              </div>
            </div>

          </div>

          {/* Partner Sales Breakdown Table */}
          <div className="bg-[#0e1017] border border-white/[0.08] rounded-xl overflow-hidden shadow-sm">
            <div className="p-4 border-b border-white/[0.08] flex items-center justify-between">
              <div>
